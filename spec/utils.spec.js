...
const { formatDate, createRefObj, formatComments } = require("../utils/index");

describe("formatDate()", () => {
  it("changes the date from a unix timestamp to a sql acceptable date", () => {
...
describe("createRefObj()", () => {
  it("takes the comments data and the inserted articles data as arguments and creates an object using the belongs_to value from the comments as the keys and the article_id values from the articles data as the values", () => {
    ...
    const actual = createRefObj(input, input2);
    const expected = {
...
describe("formatComments()", () => {
  it("converts the comments data into a format accepted by the comments table using the reference object created by createRefObj()", () => {
    const input = commentsData;
    let input2 = articlesData;
    input2.forEach((article, index) => {
      article.article_id = index + 1;
    });
    input2 = createRefObj(input, input2);
